...
| |PIC ICSP Interface x1
| |FPGA JTAG Interface x2
| |Console Interface x1
| |UIO Interface (UIO1~4, 17pin x4)
| |CAN Interface (Block Terminal 3pin x1)
| |SC Bus Interface (Host x1, Device x1)
| |Main Power Supply (DC Jack x1, Block Terminal 2pin x2)
| |UIO Power Supply (Block Terminal 2pin x2)
| |Power Output (VDD_UIO1, VDD_UIO2, VDD_3V3_SYS, VDD_3V3_IO, 17pin x4)
|Switch |Slide Switch for Main Power Supply x1, User Switch x1
|LED |Power LED x8, Console LED x2, User LED x1
|Tempreture Sensor |ユーザー温度センサ。ユーザースイッチによりExternal I2CでOBCと通信が可能。
...
|Board Size |200 x 130 mm (突起部を除く)
|Board Mounting Hole |穴径Φ3.2。OBC周辺の穴はCubeSat向けPC104に対応。
|===
...
|CON22 |DC/DCコンバータ出力 インタフェース |ピンヘッダ2ピン (2.54mmピッチ) |コネクタ非搭載
...
CON4はPICのICSPインタフェースであり、OBCのICSPポートへ接続されています。 +
...
CON8はUIO2のインタフェースであり、OBCのUIO2ポートへ接続されています。OBC上のFPGAへ接続されており、ユーザーが自由に使用できる信号です。
...
CON10はUIO3, TRCH UART, Watchdog出力信号, I2Cのインタフェースであり、OBCへ接続されています。 +
...
|4 |NC |- |- |未接続
.2+^.^|5 |TRCH_UART_TX |Out |VDD_3V3_SYS |TRCH UART TX信号
      |UIO4_09 |In/Out |VDD_3V3_IO |UIO4 入出力
.2+^.^|6 |TRCH_UART_RX |In |VDD_3V3_SYS |TRCH UART RX信号
...
CON11はUIO3, TRCH UART, Watchdog出力信号, I2C用の電源およびGNDのインタフェースです。 +
...
CON14はConsoleインタフェースであり、FTDI社製 https://ftdichip.com/products/ft2232h-56q/[FT2232H-56Q] を経由して、OBC
のFPGAおよびTRCHのUARTポートへ接続が可能です。 +
...
CON15はSC Busのホストインタフェースです。Data (D+/D-)ラインはOBCと接続されており、CON16と共用しています。 +
...
SC Busを使用する場合、OBCはホストとしてもデバイスとしても利用が可能です。 +
CON16はSC Busのデバイスインタフェースです。Data (D+/D-)ラインはOBCと接続されており、CON15と共用しています。 +
...
=== DC/DCコンバータ出力 インタフェース（CON22）

CON22は VDD_3V3_DCDC の出力インタフェースです。VDD_UIO1 および VDD_UIO2 の電源以外の用途での使用を想定しています。 +
最大出力電流は 2A です。
...
SSW1はOBCと以下の回路の間の接続を制御するためのスイッチです。 +
デフォルト設定はBit 3,4のみがONで、その他のBitはOFFです。使用する回路のBitを手動でONにしてください。 +
手動でONする場合、指では操作しづらいため、ピンセットなどの先の尖った道具を使用してください。
...
[width="100%",cols="1,1,2,2,2,4",options="header",]
|===
...
.2+^.^|5 |5 |I2C_EXT_SCL |Output |VDD_3V3_SYS |I2C SCL信号
      |12 |I2C_TEMP_SENS_SCL |Output |VDD_3V3_SYS |User Temp Sensor (IC7)のPin2 (SCL)へ接続
.2+^.^|6 |6 |I2C_EXT_SDA |In/Out |VDD_3V3_SYS |I2C SDA信号
...
SW2は入力保護回路 (IC11, IC12)のEN端子を制御し、CON17~19とVIN_A, VIN_Bとの間を接続または切断できるインタフェースです。 +
...
* xref:attachment$SC-MPN-0056A_Layout-A_Rev1.pdf[SC-MPN-0056A_Layout-A_Rev1.pdf]
* xref:attachment$SC-MPN-0056A_Layout-B_Rev1.pdf[SC-MPN-0056A_Layout-B_Rev1.pdf]

==== Revision History
[width="100%",cols="1,1,1,4",options="header",]
|===
|Date |File |Rev. |Description
|2025/6/16 |Layout-A |1.0 |Initial Release
|2025/6/16 |Layout-B |1.0 |Initial Release
|===