...
|MPLAB PIC KIT |Microchip製のデバッカです。TRCHのソフトウェア書き込みに使用します。
|Platform Cable USB II |Xilinx製のデバッカです。FPGAの書き込みなどに使用します。
|===
...
|===
|項目 |記号 |条件 |最小 |標準 |最大 |単位
|過電圧保護(OVP)しきい値電圧 |Vocp_th |VIN_A, VIN_B ^[Note1]^ | |5.6 | |V
|過電流保護(OCP)しきい値電流 |Iocp_th |VIN_A ^[Note1]^ |3.2 | | |A
| | |VIN_B ^[Note1]^ |1.2 | | |A
| | |VDD_3V3_SYS ^[Note1]^ |100 | | |mA
| | |VBUS_A ^[Note1]^ |1.0 | | |A
|SC Bus Host 出力電圧 |VBUS_A | | |5.0 | |V
|DC/DCコンバータ出力電圧 |VDD_3V3_DCDC |I~dcdc~ = 2A |3.135 |3.3 |3.465 |V
|DC/DCコンバータ出力電流 |I~dcdc~ | | | |2 |A
|===
...
|CON2 |OBC FPGA JTAG インタフェース |シュラウド型ライトアングル ピンヘッダ14ピン (2.0mmピッチ) |
|CON3 |OBC FPGA JTAG インタフェース (Flying Wire Adapter) |ピンヘッダ6ピン (2.54mmピッチ) |コネクタ非搭載
|CON4 |PIC ICSP インタフェース |ライトアングル ピンソケット8ピン (2.54mmピッチ) |
...
|CON22 |3.3V電源出力 インタフェース |ピンヘッダ2ピン (2.54mmピッチ) |コネクタ非搭載
...
|JP4 |電源入力切替 インタフェース (UIO1) |ピンヘッダ6ピン (2.54mmピッチ) |ジャンパーソケット搭載
|JP5 |電源入力切替 インタフェース (UIO2) |ピンヘッダ6ピン (2.54mmピッチ) |ジャンパーソケット搭載
...
|24 |GND |Power |- |電源 (GND)
|25 |UIO3_00 / UIIO4_06 |In/Out |VDD_3V3_SYS / VDD_3V3_IO ^[fn:hwopt]^ |UIO3 Bit 0信号。ハードウェアオプションで UIO4 Bit
6信号との接続変更が可能。
|26 |UIO3_01 / UIIO4_07 |In/Out |VDD_3V3_SYS / VDD_3V3_IO ^[fn:hwopt]^ |UIO3 Bit 1信号。ハードウェアオプションで UIO4 Bit
7信号との接続変更が可能。
|27 |UIO3_02 / UIIO4_08 |In/Out |VDD_3V3_SYS / VDD_3V3_IO ^[fn:hwopt]^ |UIO3 Bit 2信号。ハードウェアオプションで UIO4 Bit
8信号との接続変更が可能。
|28 |UIO4_00 |Input |VDD_3V3_IO |UIO4 Bit 0信号
|29 |GND |Power |- |電源 (GND)
|30 |TRCH_UART_TX / UIO4_09 |Input |VDD_3V3_SYS / VDD_3V3_IO ^[fn:hwopt]^ |TRCH UART TX信号。ハードウェアオプションで UIO4 Bit
9信号との接続変更が可能。
|31 |TRCH_UART_RX / UIO4_10 |Output |VDD_3V3_SYS / VDD_3V3_IO ^[fn:hwopt]^ |TRCH UART RX信号。ハードウェアオプションで UIO4 Bit 10信号との接続変更が可能。
...
|36 |WDOG_OUT / UIO4_11 |Input |VDD_3V3_SYS / VDD_3V3_IO ^[fn:hwopt]^ |Watchdog入力信号。ハードウェアオプションで UIO4 Bit
11信号との接続変更が可能。
...
[fn:hwopt] : ハードウェアオプションによって電源ドメインが異なります。『Space Cubics OBC製品マニュアル』を参照してください。

=== OBC FPGA JTAG インタフェース（CON2）

CON2はJTAGインタフェースであり、OBCのJTAGポートへ接続されています。ピン配列はXilinx社製のPlatform cable USB IIに対応しており、以下とも互換性があるため直接接続も可能です。

* Digilent社製 https://digilent.com/shop/jtag-hs2-programming-cable/[JTAG-HS2 Programming Cable]
* Digilent社製 https://digilent.com/shop/jtag-hs3-programming-cable/[JTAG-HS3 Programming Cable]
...
|3 |GND |Power |- |電源（GND） |4 |FPGA_TMS |In |VDD_3V3_IO |JTAG モード選択
|5 |GND |Power |- |電源（GND） |6 |FPGA_TCK |In |VDD_3V3_IO |JTAG クロック
|7 |GND |Power |- |電源（GND） |8 |FPGA_TDO |Out |VDD_3V3_IO |JTAG データ出力
|9 |GND |Power |- |電源（GND） |10 |FPGA_TDI |In |VDD_3V3_IO |JTAG データ入力
